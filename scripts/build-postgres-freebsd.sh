...
while getopts "v:i:g:o:u:m:c:p:s:k:r:" opt; do
    case $opt in
    v) PG_VERSION=$OPTARG ;;
    i) IMG_NAME=$OPTARG ;;
...
if [ -n "${POSTGIS_VERSION:-}" ] ; then
  echo "PostGIS is not supported yet for the FreeBSD builder!" && exit 1;
fi
...
mkdir -p "$TRG_DIR"
mkdir -p "$CACHE_DIR"
rm -f "$TRG_DIR"/postgres-freebsd*.txz "$TRG_DIR"/postgres-freebsd*.txt 2>/dev/null || true

docker run -i --rm \
    -v "${TRG_DIR}:/usr/local/pg-dist" \
    ...
    $DOCKER_OPTS "$IMG_NAME" /bin/bash -eu -c '
        ...
        run_guest_build() {
            ...
            if [ -f "/usr/local/pg-cache/postgresql-${PG_VERSION}.tar.bz2" ]; then
                log "Copying cached PostgreSQL source tarball into FreeBSD guest"
                scp $SCP_OPTS "/usr/local/pg-cache/postgresql-${PG_VERSION}.tar.bz2" "${user}@127.0.0.1:/tmp/postgresql-${PG_VERSION}.tar.bz2"
            fi

            log "Copying guest build helper"
            scp $SCP_OPTS /usr/local/pg-scripts/build-postgres-freebsd-guest.sh "${user}@127.0.0.1:/tmp/build-postgres-freebsd-guest.sh"

            log "Building PostgreSQL $PG_VERSION inside FreeBSD guest"
            if [ -n "$runner" ]; then
                ssh $SSH_OPTS "${user}@127.0.0.1" "chmod +x /tmp/build-postgres-freebsd-guest.sh && ${runner} env PG_VERSION=$PG_VERSION POSTGIS_VERSION=$POSTGIS_VERSION DIST_DIR=/tmp/pg-dist PG_SOURCE_FILE=/tmp/postgresql-${PG_VERSION}.tar.bz2 /tmp/build-postgres-freebsd-guest.sh"
            else
                ssh $SSH_OPTS "${user}@127.0.0.1" "chmod +x /tmp/build-postgres-freebsd-guest.sh && env PG_VERSION=$PG_VERSION POSTGIS_VERSION=$POSTGIS_VERSION DIST_DIR=/tmp/pg-dist PG_SOURCE_FILE=/tmp/postgresql-${PG_VERSION}.tar.bz2 /tmp/build-postgres-freebsd-guest.sh"
            fi

            log "Files produced inside FreeBSD guest:"
            ssh $SSH_OPTS "${user}@127.0.0.1" "find /tmp/pg-dist -maxdepth 1 -type f | sort"
            log "Copying staged FreeBSD artifacts back to host"
            ssh $SSH_OPTS "${user}@127.0.0.1" "tar -C /tmp/pg-dist -cf - ." | tar -C /usr/local/pg-dist -xf -
            log "Files copied back to host:"
            find /usr/local/pg-dist -maxdepth 1 -type f | sort
        }
        ...
        WORK_DIR=$(mktemp -d /tmp/freebsd-builder.XXXXXX)
        cleanup() {
            local errcode=$?
        ...
        if [ ! -f "/usr/local/pg-cache/postgresql-${PG_VERSION}.tar.bz2" ]; then
            log "Downloading PostgreSQL source archive for cache"
            curl -fsSL "https://ftp.postgresql.org/pub/source/v${PG_VERSION}/postgresql-${PG_VERSION}.tar.bz2" -o "/usr/local/pg-cache/postgresql-${PG_VERSION}.tar.bz2"
        else
            log "Using cached PostgreSQL source archive"
        fi
...
instance-id: embedded-postgres-${PG_VERSION}
local-hostname: embedded-postgres-freebsd
EOF
...
users:
  - default
  - name: builder
    gecos: Embedded Postgres Builder
    groups:
      - wheel
    shell: /bin/sh
    ssh_authorized_keys:
...
EOF
            ...
            run_guest_build builder doas
        else
            mkdir -p "$WORK_DIR/install-media/etc"
            mkdir -p "$WORK_DIR/install-media/boot"
            VM_CACHE_BASENAME=$(basename "$VM_IMAGE_URL")
            if [ ! -f "/usr/local/pg-cache/${VM_CACHE_BASENAME}" ]; then
...
#!/bin/sh
sysrc hostname=embedded-postgres-freebsd
...
EOF
            ...
            run_guest_build root ""
        fi

        log "FreeBSD build flow completed"
    '
